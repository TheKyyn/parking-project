...
/**
 * Middleware d'authentification pour les utilisateurs (users)
 *
 * Vérifie que l'utilisateur authentifié est un user (type='user' dans le JWT)
 * Injecte l'userId dans les pathParams de la requête
 */
class UserAuthMiddleware
{
    ...
    public function __invoke(HttpRequestInterface $request): ?JsonResponse
    {
        try {
            ...
            // Vérifie que le type est 'user' (ou null pour rétrocompatibilité)
            if (isset($userData['type']) && $userData['type'] !== 'user') {
                return JsonResponse::forbidden('Access forbidden: User authentication required');
            }
